...
class AdditionalFeature : AppCompatActivity() {
    ...
    fun lookup(view: View?) {
        GetMoviesByTitle().execute()
    }

    internal inner class GetMoviesByTitle : AsyncTask<Void?, Void?, Void?>() {
        ...
        override fun doInBackground(vararg p0: Void?): Void? {
            ...
            val jsonStr =
                httpHandler.makeServiceCall("https://www.omdbapi.com/?s=" + editTextTextPersonName7!!.text.toString() + "&apikey=7153abd9")
            var str = ""
            if (jsonStr != null) {
                try {
                    val jsonObj = JSONObject(jsonStr)
                    val jArray = jsonObj.getJSONArray("Search")
                    for (i in 0 until jArray.length()) {
                        val js3 = jArray.getJSONObject(i)
                        str += js3.getString("Title")
                    }
                } catch (e: JSONException) {
                    Log.e("ok", "Json parsing error: " + e.message)
                }
                val finalStr = str
                runOnUiThread { text!!.text = finalStr }
            } else {
